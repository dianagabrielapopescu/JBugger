...
2.  [UserModel Interfaces and Layouts](#layouts)
...
- **Value** (Depends on the prompt)
  - `confirm`: (Boolean)
  - `input` : UserModel input (filtered if `filter` is defined) (String)
  - `number`: UserModel input (filtered if `filter` is defined) (Number)
...
## UserModel Interfaces and layouts
