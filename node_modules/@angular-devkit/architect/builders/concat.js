...
exports.default = src_1.createBuilder((options, context) => {
    ...
    return rxjs_1.from(allRuns).pipe(operators_1.concatMap(fn => stop ? rxjs_1.of(null) : rxjs_1.from(fn()).pipe(operators_1.switchMap(run => run === null ? rxjs_1.of(null) : run.outputField.pipe(operators_1.first())))), operators_1.map(output => {
        context.reportProgress(i++, allRuns.length);
